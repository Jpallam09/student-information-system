<?php
/**
 * STUDENT SUBMISSION UPDATE HANDLER
 * Allows students to update their submitted task
 */
...
if (!isset($_SESSION['student_id'])) {
    echo json_encode(['success' => false, 'message' => 'Please login to update submissions']);
    exit();
}
...
$checkExisting = mysqli_query($conn, "
    SELECT id, file_path, original_filename FROM task_submissions 
    WHERE task_id = $taskId AND student_id = $student_id
");

if (!$checkExisting || mysqli_num_rows($checkExisting) == 0) {
    echo json_encode(['success' => false, 'message' => 'No submission found. Please submit first.']);
    exit();
}

$existingSubmission = mysqli_fetch_assoc($checkExisting);
$filePath = $existingSubmission['file_path'];
$originalFileName = $existingSubmission['original_filename'];

// Handle file upload (optional - if no new file, keep existing)
